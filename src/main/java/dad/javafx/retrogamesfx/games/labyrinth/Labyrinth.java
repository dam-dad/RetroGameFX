...
public class Labyrinth extends World {
	...
	public Labyrinth() {
		super("Labyrinth", "/fxml/Labyrinth.fxml", 800, 600);
	}
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		encontrarF();
		crearMapa();
		pelota = new Player(this, (x0) * anchoBloque, (y0) * altoBloque, anchoBloque, altoBloque);
		x = x0;
		y = y0;
		...
		canvas.setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				case ENTER:
					...
					if (aux != 1)
						crearMapa();
					pelota = new Player(Labyrinth.this, (x0) * anchoBloque, (y0) * altoBloque, anchoBloque, altoBloque);
					run(gc);
					break;
				case ESCAPE:
					App.goToMain();
...
//--------------Metodos heredados--------------//
	...
	private void bloquearCamino() {
		...
		do {
			...
			// La generacion puede dar errores de colocacion de
			// casillas asi que esto es para evitar caer en bucles tras proteger las
			// casillas
			if (I > 50)
				break;
...
//-------------------Metodos de movimiento-----------------------//
	...
	public void checkAvance(int columna, int fila, char direccion2) {
		int aux2 = check(), aux3 = 1, cantidad = 0;
		// direccion = ' ';
		direccion2 = invertir(direccion2);
